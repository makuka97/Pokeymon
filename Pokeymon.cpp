...
std::string getDescription(const Pokeymon& p) {
    std::string desc = "Name " + p.name + ", Type: " + typeToString(p.type) + ", Health: "  + std::to_string(p.currentHealth) + "/" + to_string(p.maxealth) + ", Atk: " + to_string(p.attack) + " Def " + to_string(p.defense);
    return desc;
}
