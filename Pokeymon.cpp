...
std::string getDescription(const Pokeymon& p) {
    std::string desc = "Name " + p.name + ", Type: " + std::typeToString(p.type) + ", Health: "  + to_string(p.currentHealth) + "/" + to_string(p.maxealth) + ", Atk: " + to_string(p.attack) + " Def " + to_string(p.defense);
    return desc;
}
